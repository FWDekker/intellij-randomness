 ...
 *
 * @param width the preferred width of [textField]
 * @param filter the filter to apply to [textField]
 */
class VariableLabelRadioButton(
    ...
    /**
     * The current label, i.e. the contents of [textField].
     */
    var label: String
        get() = textField.text
        set(value) {
    ...
    /**
     * Adds [listener] as a listener to [button] and [textField].
     *
     * @param listener the function to invoke whenever [button] or [textField] changes
     */
    fun addChangeListener(listener: () -> Unit) = addChangeListenerTo(button, textField, listener = listener)


    /**
     * Enables or disables [button] and [textField].
     *
