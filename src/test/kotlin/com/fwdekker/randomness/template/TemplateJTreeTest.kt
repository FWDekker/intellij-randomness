...
object TemplateJTreeTest : FunSpec({
    ...
    beforeEach {
        ...
        tree = runEdt { TemplateJTree(originalList, currentList) }
    ...
    context("selectedNodeNotRoot") {
        ...
        test("returns the selected node otherwise") {
            ...
            runEdt { tree.selectionRows = intArrayOf(0) }
    ...
    context("selectedScheme") {
        context("get") {
            ...
            test("returns the selected scheme otherwise") {
                ...
                runEdt { tree.selectionPath = tree.myModel.getPathToRoot(StateNode(template)) }
        ...
        test("selects the scheme by its UUID") {
            ...
            runEdt { tree.selectedScheme?.uuid } shouldBe copy.uuid
            runEdt { tree.selectedScheme } shouldNotBe copy
        }
    }
    ...
    context("selectedTemplate") {
        context("get") {
            ...
            test("returns the selected template otherwise") {
                runEdt { tree.selectionRows = intArrayOf(1) }
        ...
        context("set") {
            test("removes the selection if null is set") {
                runEdt { tree.selectedTemplate = null }

                tree.selectionCount shouldBe 0
            }

            test("removes the selection if the template cannot be found") {
                runEdt { tree.selectedTemplate = Template("New Template") }

                tree.selectionCount shouldBe 0
            }

            test("selects the given template if it exists") {
                runEdt { tree.selectedTemplate = currentList.templates[2] }
    ...
    context("reload") {
        test("synchronizes removed nodes") {
            runEdt { currentList.templates.remove(currentList.templates[2]) }
            runEdt { tree.rowCount } shouldBe 3

            runEdt { tree.reload() }

            runEdt { tree.rowCount } shouldBe 2
        }

        test("synchronizes added nodes") {
            runEdt { currentList.templates += Template("New Template", mutableListOf(DummyScheme("New Scheme"))) }
            runEdt { tree.rowCount } shouldBe 3

            runEdt { tree.reload() }

            runEdt { tree.rowCount } shouldBe 5 // Including new scheme
        }
        ...
        test("keeps collapsed nodes collapsed") {
            runEdt { tree.collapseRow(3) }
            runEdt { tree.isCollapsed(3) } shouldBe true
        ...
        test("keeps expanded nodes expanded") {
            runEdt { tree.expandRow(0) }
            runEdt { tree.isExpanded(0) } shouldBe true
        ...
        test("expands new templates") {
            ...
            runEdt { currentList.templates += template }

            runEdt { tree.reload() }

            runEdt { (tree.getPathForRow(3).lastPathComponent as StateNode).state } shouldBe template
            runEdt { tree.isExpanded(3) } shouldBe true
        }
        ...
        test(
            ...
            runEdt { tree.isCollapsed(0) } shouldBe true
        }
    }
    ...
    context("expandAll") {
        test("expands all templates") {
            ...
            runEdt { tree.expandAll() }

            runEdt { tree.rowCount } shouldBe 8
        }
    }
    ...
    context("addScheme") {
        context("unique name") {
            test("appends (1) if a template with the given name already exists") {
                runEdt { tree.addScheme(Template("Template0")) }
        ...
        context("template") {
            ...
            test("expands the inserted template") {
                ...
                runEdt { tree.addScheme(template) }
        ...
        context("scheme") {
            test("fails if a scheme is inserted while nothing is selected") {
                ...
                shouldThrow<IllegalArgumentException> { runEdt { tree.addScheme(DummyScheme()) } }
            ...
            test("inserts the scheme at the bottom of the selected template") {
                ...
                runEdt { tree.addScheme(DummyScheme("New Scheme")) }
            ...
            test("selects the inserted scheme") {
                ...
                runEdt { tree.addScheme(DummyScheme()) }
            ...
            test("keeps the parent expanded") {
                ...
                runEdt { tree.isExpanded(1) } shouldBe true
            }
    ...
    context("removeScheme") {
        test("removes the given node from the tree") {
            runEdt { tree.removeScheme(currentList.templates[1]) }
        ...
        test("removes the selection if there is nothing to select") {
            runEdt { repeat(3) { tree.removeScheme(currentList.templates[0]) } }

            tree.selectedScheme shouldBe null
        }

        test("selects the parent if the removed node had no other siblings") {
            runEdt { tree.expandRow(1) }

            runEdt { tree.removeScheme(currentList.templates[1].schemes[0]) }
        ...
        test("selects the next sibling if the removed node has siblings") {
            ...
            runEdt { tree.removeScheme(currentList.templates[0].schemes[0]) }
    ...
    context("replaceScheme") {
        test("removes the scheme if replaced with `null`") {
            ...
            runEdt { tree.replaceScheme(scheme, null) }
        ...
        test("replaces the node with another node") {
            ...
            runEdt { tree.replaceScheme(oldScheme, newScheme) }
        ...
        test("retains the current selection") {
            runEdt { tree.selectedScheme = currentList.templates[2] }

            runEdt { tree.replaceScheme(currentList.templates[1].schemes[0], DummyScheme()) }

            runEdt { tree.selectedScheme } shouldBe currentList.templates[2]
        }
        ...
        test("keeps the replaced template collapsed") {
            ...
            runEdt { tree.collapseRow(0) }

            runEdt { tree.replaceScheme(oldTemplate, newTemplate) }
        ...
        test("keeps the parent expanded") {
            ...
            runEdt { tree.replaceScheme(currentList.templates[0].schemes[1], DummyScheme()) }
    ...
    context("moveSchemeByOnePosition") {
        context("template") {
            test("moves a template") {
                runEdt { tree.moveSchemeByOnePosition(currentList.templates[1], moveDown = true) }
            ...
            test("selects the moved template") {
                ...
                runEdt { tree.selectedTemplate = template }
                runEdt { tree.moveSchemeByOnePosition(template, moveDown = false) }
            ...
            test("keeps the moved template collapsed") {
                ...
                runEdt { tree.isCollapsed(1) } shouldBe true
            }
        }
        ...
        context("scheme") {
            test("moves a scheme") {
                ...
                runEdt { tree.moveSchemeByOnePosition(currentList.templates[0].schemes[1], moveDown = false) }
            ...
            test("retains the selection after moving a scheme") {
                ...
                runEdt { tree.moveSchemeByOnePosition(scheme, moveDown = true) }
            ...
            test("if the parent changes, keeps the old parent expanded and expands the new parent") {
                ...
                runEdt { tree.isExpanded(2) } shouldBe true
            }
        }
    }
    ...
    context("canMoveSchemeByOnePosition") {
        ...
        withTests(
            mapOf<String, Tuple3<() -> Scheme, Boolean, Boolean>>(
                "first template cannot move up" to tuple({ currentList.templates[0] }, false, false),
                "first scheme cannot move up" to tuple({ currentList.templates[0].schemes[0] }, false, false),
                "last template cannot move down" to tuple({ currentList.templates[2] }, true, false),
                "last scheme cannot move down" to tuple({ currentList.templates[2].schemes[1] }, true, false),
                "template can move within list" to tuple({ currentList.templates[1] }, false, true),
                "second-to-last template can move down" to tuple({ currentList.templates[1] }, true, true),
                "scheme can move within template" to tuple({ currentList.templates[0].schemes[1] }, false, true),
                "scheme can go to previous template" to tuple({ currentList.templates[2].schemes[0] }, false, true),
                "scheme can go to next template" to tuple({ currentList.templates[1].schemes[0] }, true, true),
            )
    ...
    context("buttons") {
        beforeEach {
            frame.cleanUp()
            frame = showInFrame(runEdt { tree.asDecoratedPanel() })
        ...
        context("RemoveButton") {
            ...
            test("removes the selected scheme") {
                ...
                runEdt { frame.getActionButton("Remove").click() }
        ...
        context("CopyButton") {
            ...
            test("copies the selected scheme") {
                ...
                runEdt { frame.getActionButton("Copy").click() }
        ...
        context("UpButton") {
            ...
            test("moves the selected scheme up") {
                ...
                runEdt { frame.getActionButton("Up").click() }
        ...
        context("DownButton") {
            ...
            test("moves the selected scheme down") {
                ...
                runEdt { frame.getActionButton("Down").click() }
        ...
        context("ResetButton") {
            ...
            test("resets changes to the initially selected scheme") {
                ...
                runEdt { frame.getActionButton("Reset").click() }
